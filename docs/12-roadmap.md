...
| 7 — Geostatistics | **Started.** `13` §19's build order, step 1: the §16 flag registry and the `prep/` pipeline — validate, decluster, transform, screen. Variography, the kriging engine and REML are next, and §19 is emphatic that REML comes **before** the trend loop, which is unsafe without it. |
