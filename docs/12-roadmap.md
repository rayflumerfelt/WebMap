...
| 7 — Geostatistics | **Started.** `13` §19's build order, steps 1–5: the §16 flag registry; the `prep/` pipeline; Matérn and stable models with nested structures and §7.6's anisotropy significance test; simple and block kriging; **REML** with both tests `adr/0011` calls non-optional; and §8's trend machinery — preset forms, a sandboxed expression parser, the GLS loop and all four of §8.4's failure modes. **RK is next**, which §19 calls the first genuinely useful deliverable. |
