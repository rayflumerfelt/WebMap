...
| 7 — Geostatistics | **Started.** `13` §19's build order, steps 1–3: the §16 flag registry; the `prep/` pipeline (validate, decluster, transform, screen); Matérn and stable models with nested structures and §7.6's anisotropy significance test; and simple and block kriging on the shared solve path, with singular systems counted rather than announced per node. **REML is next**, and §19 is emphatic that it comes before the trend loop, which is unsafe without it. |
