...
| 7 — Geostatistics | **In progress.** `13` §19's build order, steps 1–6: the §16 flag registry; the `prep/` pipeline; Matérn and stable models, nested structures and §7.6's anisotropy significance test; simple and block kriging; **REML** with both tests `adr/0011` calls non-optional; §8's trend machinery — preset forms, a token-validated expression parser, the GLS loop and all four of §8.4's failure modes; and **regression kriging**, which §19 calls the first genuinely useful deliverable. Indicator machinery and RIK are next. |
