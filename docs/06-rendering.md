...
**Built**, with `tests/visual/golden/` deliberately empty. Four things about how:

**A missing golden skips, it does not pass.** A parametrised test that quietly does nothing
when its reference image is absent is worse than no test — the suite goes green and nobody
knows the case is unprotected. Every case here skips with the command that would make one and
the reminder to look at it.

**The fixtures are self-contained.** Every source is inline GeoJSON generated by
`tests/visual/fixtures/generate.py`, so a case renders with no API, no tile server and no
object store in reach. That is not only convenience: the render service refuses hosts it does
not allow, and a fixture that fetched anything would make this a test of the network.

**A render with failed requests fails on the failure, not on the pixels.** A map missing a
layer differs from its golden everywhere, and "37% of pixels differ" describes that far worse
than the URL that would not load.

**The master image is compared, not the preview.** The preview is downscaled to 1600 px for an
MCP response (`adr/0006`), and a change smaller than the resampling kernel would pass.

`tests/e2e/` is the other half — Playwright against the running application at 1440×900, `07`
§5.1's design width. It asserts what only a browser can: that the grid template renders the
layout it describes, that the minimum-width notice replaces the shell below 1280 px, that the
1920 px breakpoint widens the panels, and that Tab reaches something. Its `page` fixture
**fails a test on any console error**, because a React error boundary leaves a page that looks
almost right and an assertion on what is visible passes over a broken application.
